...
export async function runValidationContract({ template, appPath, baseUrl }) {
  ...
  for (const item of profile.checks) {
    ...
    if (!fn) {
      // Unknown check id is a deterministic configuration/profile issue.
      // Classify as SCHEMA_FAIL (not UNKNOWN_FAIL).
      const res = {
        id,
        required,
        ok: !required,
        class: required ? ValidationClass.SCHEMA_FAIL : null,
        details: {
          code: "ERR_UNKNOWN_CHECK_ID",
          message: "Profile referenced an unknown check id.",
